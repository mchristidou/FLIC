        ...
        <div class="container my-2">
            <div class="row g-4">
                <div class="col-10 col-sm-8">
                    <?php $sql = "SELECT review_id, user_id, title, reviews.rating, review_text, movies.movie_id as id, movies.imdb_id
                ...
                <div class="col-9 col-sm-4">
                    ...
                    $query->execute(array(':review_id'=>$review_id));
                    $record = $query->fetch(); ?>
                    <a href="movie_page.php?search_value=<?php echo $record['title']; ?>&movie_id=<?php echo $record['id']; ?>">
                        <?php $url = "http://www.omdbapi.com/?apikey=" . $apiKey . "&i=" . $record['imdb_id'];
                        $response = file_get_contents($url);
                        $movie_data = json_decode($response, true); ?>
                        <?php if (!empty($movie_data['Poster'])) { ?>
                            <img src="<?php echo $movie_data['Poster'];?>" class="movie-poster poster-img" alt="<?php echo $movie_data['Title'];?>">
                        <?php } ?>
                    </a>
                </div>
            </div>
